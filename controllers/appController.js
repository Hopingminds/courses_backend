...
export async function register(req, res) {
  try {
    ...
    if (mailSent) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to send registration email" });
    }
  } catch (error) {
    res.status(500).json({
      success: false,
...
export async function registerSwayam(req, res) {
  try {
    const {
      password,
      name,
...
export async function getUser(req, res) {
  ...
  try {
    ...
    const user = await UserModel.findOne({ email })
      ...
      .populate({ path: "purchased_internships.internship" })
      .exec();
...
export async function validateFields(req, res) {
  try {
    ...
    if (!email && !phone) {
      return res
        .status(400)
        .send({ success: false, error: "Email or phone must be provided" });
    }
    ...
    return res
      .status(200)
      .send({ success: true, msg: "Email and phone are available" });
  } catch (error) {
...
export async function deleteAccount(req, res) {
  try {
    ...
    if (!usertodelete) {
      usertodelete = new AccDeleteReqModel({
        user: userID,
      ...
      return res
        .status(400)
        .json({ success: false, message: "Request has already made" });
    } else if (usertodelete && usertodelete.requestCancelled) {
    ...
    setTimeout(async () => {
      try {
        ...
        const currentRequest = await AccDeleteReqModel.findOne({
          user: userID,
        });
    ...
    return res
      .status(200)
      .json({ success: true, message: "Account will be deleted in 24 hours" });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Internal server error" + error.message,
...
export async function cancelAccountDeletion(req, res) {
  try {
    ...
    if (!deletionRequest) {
      return res
        .status(404)
        .json({ success: false, message: "No account deletion request found" });
    }
    ...
    if (deletionRequest.requestCancelled) {
      return res.status(400).json({
        success: false,
        message: "Deletion request has already been canceled",
      });
    }
...
export async function addCourse(req, res) {
  try {
    ...
    if (
      !user.purchased_courses.some(
        (purchasedCourse) =>
          purchasedCourse.course && purchasedCourse.course.equals(courseId)
      )
    ) {
    ...
    await registerMail(
      {
        body: {
          username: user.name,
          userEmail: email,
          subject: `Success! You have successfully enrolled in the course`,
          text: `You have successfully enrolled in ${course.title} course.`,
        },
      },
      {
        status(status) {
    ...
    if (mailSent) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to enrol course." });
    }
  } catch (error) {
    res.status(500).json({
      success: false,
